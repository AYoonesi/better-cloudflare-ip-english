...
echo "https://www.arvancloud.ir/fa/ips.txt Save as ips-v4-arvan.txt"
...
do
	...
	elif [ ! -f "url-arvan.txt" ]
	then
		echo "Download the speed test file address from the server url.txt"
		curl --retry 2 -s https://www.baipiao.eu.org/cloudflare/url -o url-arvan.txt
	elif [ ! -f "ips-v4-arvan.txt" ]
	then
		echo "Download IPV4 data from server ips-v4-arvan.txt"
		curl --retry 2 -s https://www.arvancloud.ir/fa/ips.txt -o ips-v4-arvan.txt
	elif [ ! -f "ips-v6.txt" ]
	then
...
do
	...
	if [ $menu == 8 ]
	then
		rm -rf colo.txt url.txt ips-v4-arvan.txt ips-v6.txt
		datacheck
		clear
	fi
done
